...
async function loadProducts() {
  ...
  querySnapshot.forEach((doc) => {
    ...
    li.textContent = `${product.name} - Vencimento: ${product.expirationDate}`;
    ...
    if (daysUntilExpiration <= 7 && daysUntilExpiration >= 0) {
      li.classList.add("alert");  // Adiciona a classe de alerta vermelho
      alertMessage = ` (Vencimento em ${daysUntilExpiration} dias!)`;
      alertsDiv.innerHTML += `<div class="alert-red">Alerta: Produto ${product.name} vencendo em ${daysUntilExpiration} dias!</div>`;
      hasAlert = true;
    } else if (daysUntilExpiration <= 30 && daysUntilExpiration > 7) {
      li.classList.add("alert");
      alertMessage = ` (Vencimento em ${daysUntilExpiration} dias!)`;
      alertsDiv.innerHTML += `<div class="alert-yellow">Alerta: Produto ${product.name} vencendo em ${daysUntilExpiration} dias!</div>`;
      hasAlert = true;
    } else if (daysUntilExpiration <= 60 && daysUntilExpiration > 30) {
      li.classList.add("alert");
      alertMessage = ` (Vencimento em ${daysUntilExpiration} dias!)`;
      alertsDiv.innerHTML += `<div class="alert-green">Alerta: Produto ${product.name} vencendo em ${daysUntilExpiration} dias!</div>`;
      hasAlert = true;
    }
  ...
  if (!hasAlert) {
    alertsDiv.innerHTML += `<div class="alert-green">Nenhum produto com validade próxima.</div>`;
  }
}
